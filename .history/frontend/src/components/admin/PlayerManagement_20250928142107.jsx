  ...
  if (error) {
    return (
      <div className="error-container">
        <h3>❌ Có lỗi xảy ra</h3>
        <p>{error}</p>
        <button onClick={loadPlayers} className="retry-btn">
          Thử lại
        </button>
      </div>
    );
    ...
    <div className="player-management">
      ...
      <div className="players-list">
        {players && players.length > 0 ? (
          players.map(player => {
            const latestGrade = getScoreGrade(player.latestScore || player.highestScore || 0);
            return (
              <div key={player._id || player.name} className="player-card">
                <div className="player-header">
                  <div className="player-name">{player._id || player.name}</div>
                ...
                <div className="player-stats">
                  <div className="stat-item">
                    ...
                    <span className="stat-value">{player.totalGames || 0}</span>
                  </div>
                  <div className="stat-item">
                  ...
                  <div className="stat-item">
                    ...
                    <span className="stat-value">{(player.averageScore || 0).toFixed(1)}</span>
                  </div>
                  <div className="stat-item">
                    <span className="stat-label">Lần cuối:</span>
                    <span className="stat-value">
                      {player.latestPlayed ? new Date(player.latestPlayed).toLocaleDateString('vi-VN') : 'N/A'}
                    </span>
                  </div>
                </div>
                ...
                <div className="player-actions">
                  <button 
                    onClick={() => handleViewPlayer(player._id || player.name)}
                    className="view-btn"
                  >
                    👁️ Xem chi tiết
                  </button>
          ...
          <div className="no-data">
            ...
            {searchTerm && (
              <p className="no-data-subtitle">Thử tìm kiếm với từ khóa khác.</p>
            )}
          </div>
        )}
      </div>
