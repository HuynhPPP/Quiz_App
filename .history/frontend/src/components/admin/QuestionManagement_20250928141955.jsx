  ...
  const fetchQuestions = async (page = currentPage, search = searchQuery, category = selectedCategory) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        setQuestions(response.data.docs || response.data || []);
        setTotalPages(response.data.totalPages || response.pagination?.totalPages || 1);
        setCurrentPage(response.data.page || response.pagination?.currentPage || 1);
      } else {
        ...
        <div className="filter-group">
          ...
          <select id="categoryFilter" value={selectedCategory} onChange={handleCategoryChange}>
            ...
            {categories.map((cat, index) => (
              <option key={cat._id || `cat-${index}`} value={cat.name}>{cat.emoji} {cat.name}</option>
            ))}
          </select>
        </div>
            ...
            <form onSubmit={handleFormSubmit}>
              <div className="form-group">
                ...
                >
                  ...
                  {categories.map((cat, index) => (
                    <option key={cat._id || `cat-modal-${index}`} value={cat.name}>{cat.emoji} {cat.name}</option>
                  ))}
                </select>
              </div>
              <div className="form-group">
